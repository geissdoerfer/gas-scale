...
void app_main(void) {
  ...
  hx711_t dev = {.dout = 5, .pd_sck = 4, .gain = HX711_GAIN_A_64};
  ...
  while (1) {
    ...
    if (r != ESP_OK) {
      ESP_LOGE(TAG, "Device not found: %d (%s)", r, esp_err_to_name(r));
      vTaskDelay(1000 / portTICK_PERIOD_MS);
      continue;
    }

    // Read data from HX711 (average of 5 readings)
    int32_t raw_data;
    r = hx711_read_average(&dev, 5, &raw_data);
    if (r != ESP_OK) {
